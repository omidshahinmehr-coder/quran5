...
class QuranRepository(private val context: Context) {
    ...
    suspend fun getAllTranslations(): Map<Int, TranslationEntity> = withContext(Dispatchers.IO) {
    ...
    suspend fun getTranslations(startId: Int, endId: Int): List<TranslationEntity> = withContext(Dispatchers.IO) {
        val result = mutableListOf<TranslationEntity>()
        val cursor = db.rawQuery(
            "SELECT globalAyahId, translator, textFa FROM translation " +
                "WHERE globalAyahId BETWEEN ? AND ?",
            arrayOf(startId.toString(), endId.toString())
        )
        cursor.use {
            while (it.moveToNext()) {
    ...
    suspend fun getTafsirForSurah(surah: Int): List<TafsirEntity> = withContext(Dispatchers.IO) {
        val result = mutableListOf<TafsirEntity>()
        val cursor = db.rawQuery(
            "SELECT id, source, surahNumber, startAyahId, endAyahId, textFa FROM tafsir " +
                "WHERE surahNumber = ? ORDER BY startAyahId",
            arrayOf(surah.toString())
        )
        cursor.use {
            while (it.moveToNext()) {
    ...
    suspend fun getTafsirForAyah(globalAyahId: Int): List<TafsirEntity> = withContext(Dispatchers.IO) {
        val result = mutableListOf<TafsirEntity>()
        val cursor = db.rawQuery(
            "SELECT id, source, surahNumber, startAyahId, endAyahId, textFa FROM tafsir " +
                "WHERE ? BETWEEN startAyahId AND endAyahId ORDER BY startAyahId",
            arrayOf(globalAyahId.toString())
        )
        cursor.use {
            while (it.moveToNext()) {
                result += TafsirEntity(
